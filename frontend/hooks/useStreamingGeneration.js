...
/**
 * Hook for WebSocket-based streaming melody generation with real-time Tone.js playback.
 */
export function useStreamingGeneration() {
  ...
  const playNote = useCallback((noteEvent) => {
  ...
  const start = useCallback(async (modelId, instrument, options = {}) => {
    setIsStreaming(true);
    setProgress(0);
    setTotalNotes(0);
    setNotes([]);
    ...
    const connection = createStreamingConnection({
      ...
      onClose: () => {
        // Surface disconnect if still streaming
        if (connectionRef.current) {
          setIsStreaming(false);
        }
      },
