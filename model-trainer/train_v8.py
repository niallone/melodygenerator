"""
train_v8.py — MusicTransformer v8 trained on MAESTRO + v5_various.

Same architecture as v7 (RoPE + SwiGLU + RMSNorm), larger BPE vocab for richer dataset.

Usage:
    python train_v8.py [--input-dir /path/to/midi] [--output-dir /path/to/output]
                       [--maestro-dir /path/to/maestro] [--model-name melody_generator_transformer_v8]
"""
...
sys.path.insert(0, os.path.join(_here, "model-trainer"))
...
# Training — more warmup for larger dataset
EPOCHS = 100
BATCH_SIZE = 64
LEARNING_RATE = 3e-4
WEIGHT_DECAY = 0.01
...
SEQUENCE_LENGTH = 256
STRIDE = 64
BPE_VOCAB_SIZE = 1024  # Larger vocab for richer MAESTRO data

# Defaults
DEFAULT_INPUT_DIR = "/app/input"
DEFAULT_OUTPUT_DIR = "/app/model"
DEFAULT_MODEL_NAME = "melody_generator_transformer_v8"


def parse_args():
    parser = argparse.ArgumentParser(description="Train MusicTransformer v8 (MAESTRO)")
...
async def train():
    ...
    print(f"Maestro directory: {args.maestro_dir}")
    ...
    print(f"BPE training on {len(midi_files)} files...")
